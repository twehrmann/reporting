...
calcErrorProp <- function(inputData) {
  BaseCruces<-0
  BaseCruces<-inputData@BaseCruces
  length(BaseCruces$Value)
  
  BaseDinamica<-0
  BaseDinamica<-inputData@BaseDinamica
  length(BaseDinamica$combinacion)
  
  TablaFEdefor<-0
  TablaFEdegra<-0
  TablaFAperma<-0
  TablaFArecup<-0
  TablaFEdefor<-inputData@TablaFEdefor
  TablaFEdeforPrad<-TablaFEdefor
  TablaFEdegra<-inputData@TablaFEdegra
  TablaFArecup<-inputData@TablaFArecup
  TablaFArecupPrad<-TablaFArecup
  TablaFArefo<-TablaFArecup
  #Se identifica el estrato permamnete 
  TablaFApermaP<-inputData@TablaFApermaP
